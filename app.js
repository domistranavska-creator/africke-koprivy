...
function normalizeLoadedData(parsed) {
  ...
  data.trash = data.trash.map(normalizeTrashEntry);
...
function trashEntryMeta(entry = {}) {
  const deletedLabel = clean(entry.deletedAt) ? formatCloudDateTime(entry.deletedAt) : "";
  const type = clean(entry.type);
...
function normalizeVarietySnapshot(variety = {}) {
  const now = new Date().toISOString();
  return {
    ...
    priceHistory: normalizePriceHistory(variety.priceHistory || initialPriceHistoryForVariety(variety, now)),
...
function restoreTrashEntry(entryId) {
  ...
  if (entry.type === "customer-bundle") {
    const customer = sanitizeCustomer(cloneTrashPayload(entry?.payload?.customer || {}));
    customer.updatedAt = restoredAt;
    if (!clean(customer.id)) {
      toast("Zákazníka se nepodařilo obnovit.");
      return;
    }
    upsertRestoredRecord("customers", customer);
    (entry?.payload?.orders || []).forEach((order) => {
      const restoredOrder = normalizeOrder(cloneTrashPayload(order));
      restoredOrder.updatedAt = restoredAt;
      upsertRestoredRecord("orders", restoredOrder);
      state.selectedOrderIds.delete(restoredOrder.id);
    });
    state.data.customers = mergeDuplicateCustomers(state.data.customers, state.data.orders);
    state.selectedCustomerId = findCustomer(customer.id)?.id || state.data.customers[0]?.id || null;
  } else if (entry.type === "order") {
    const order = normalizeOrder(cloneTrashPayload(entry?.payload?.order || {}));
    order.updatedAt = restoredAt;
    upsertRestoredRecord("orders", order);
    state.selectedOrderIds.delete(order.id);
  } else if (entry.type === "variety") {
    const variety = normalizeVarietySnapshot(cloneTrashPayload(entry?.payload?.variety || {}));
    variety.updatedAt = restoredAt;
    upsertRestoredRecord("varieties", variety);
  } else if (entry.type === "cross") {
    const cross = normalizeCross(cloneTrashPayload(entry?.payload?.cross || {}));
    cross.updatedAt = restoredAt;
    upsertRestoredRecord("crosses", cross);
    state.selectedCrossId = cross.id;
  } else if (entry.type === "offer") {
    const offer = normalizeOffer(cloneTrashPayload(entry?.payload?.offer || {}));
    offer.updatedAt = restoredAt;
    upsertRestoredRecord("offers", offer);
    state.selectedOfferId = offer.id;
  } else if (entry.type === "offer-item") {
    const offer = findOffer(entry?.payload?.offerId);
    if (!offer) {
    ...
    toast("Tento záznam se zatím nedá obnovit.");
    return;
  }
...
function renderTrashPanel() {
  ...
  listEl.querySelectorAll("[data-trash-restore]").forEach((button) => {
    button.addEventListener("click", () => restoreTrashEntry(button.dataset.trashRestore));
  });
  listEl.querySelectorAll("[data-trash-delete]").forEach((button) => {
    button.addEventListener("click", () => permanentlyDeleteTrashEntry(button.dataset.trashDelete));
  });
}
...
function customerOrders(customerId = "") {
  ...
  if (!id) return [];
  return (state.data.orders || []).filter((order) => clean(order.customerId) === id);
}

function customerAvatarMeta(customer = {}, stornoMeta = customerStornoMeta(customer.id), nonPaymentMeta = customerNonPaymentMeta(customer.id)) {
  const orders = customerOrders(customer.id);
  const allPaid = orders.length > 0 && orders.every((order) => parsePaymentStatus(order.paymentStatus) === "zaplaceno" && !orderHasStorno(order));
  if (nonPaymentMeta.count || stornoMeta.count) {
    return { image: "assets/Snímka obrazovky 2026-06-11 123125.png", label: "Pozor, problémový zákazník", className: "customer-avatar-angry" };
  }
  if (orders.length > 1 && allPaid) {
    return { image: "assets/Snímka obrazovky 2026-06-11 123119.png", label: "Stálý spokojený zákazník", className: "customer-avatar-happy" };
  }
  if (orders.length > 0) {
    return { image: "assets/Snímka obrazovky 2026-06-11 123112.png", label: "Už nakoupil", className: "customer-avatar-positive" };
  }
  return { image: "assets/Snímka obrazovky 2026-06-11 123130.png", label: "Zatím bez nákupu", className: "customer-avatar-neutral" };
}
  ...
  els.customersTable.innerHTML = customers.length
    ? customers
        .map((customer) => {
          ...
          const avatar = customerAvatarMeta(customer, stornoMeta, nonPaymentMeta);
          const toneClass = avatar.className.replace("customer-avatar", "customer-tone");
          return `<tr class="${[
            ...
            <td>
              <span class="customer-main-with-avatar">
                <span class="customer-avatar ${avatar.className}" title="${escapeHtml(avatar.label)}" aria-label="${escapeHtml(avatar.label)}"><img class="customer-avatar-image" src="${escapeHtml(avatar.image)}" alt="" aria-hidden="true" decoding="async" loading="lazy"></span>
                <span class="cell-main">${escapeHtml(customerName(customer))}</span>
              </span>
              <span class="cell-sub">${escapeHtml([customer.fbName ? `FB: ${customer.fbName}` : "", customerRatingLabel(customer)].filter(Boolean).join(" · "))}</span>
            </td>
            <td>
              ...
              ${customer.note ? `<span class="customer-contact-note">${escapeHtml(shortRestText(customer.note, 92))}</span>` : ""}
...
function crossActorThumb(image, label, extraAttrs = "") {
  if (image) return `<span class="cross-actor-thumb">${photoImageMarkup(image, label, "", `loading="lazy" ${extraAttrs}`)}</span>`;
  return `<span class="cross-actor-thumb empty">${escapeHtml(varietyInitials(label || "K"))}</span>`;
}

function crossSeedlingImages(cross = {}) {
  const linkedImages = varietyImages(crossSeedlingVariety(cross));
...
function crossSeedlingMainPhoto(cross = {}) {
  return clean(globalThis.AKSyncPhotoCore?.crossPhoto(cross || {}, state.data)) || crossSeedlingImages(cross)[0] || "";
}
...
function buildCrossPreviewMarkup(cross, options = {}) {
  ...
  const seedlingVariety = crossSeedlingVariety(cross);
  ...
  return `
    <div class="cross-preview-flow">
      ...
      <article class="cross-actor accent">
        ${crossActorThumb(seedlingPhotoUrl, seedlingName, photoFallbackAttributes(seedlingVariety, seedlingImages))}
        <small>Semenáč</small>
...
function renderOfferItem(offer, item) {
  ...
  const orderFlag = orderProgress.label
    ? `<span class="offer-order-flag offer-order-flag-inline offer-order-flag-${escapeHtml(orderProgress.state)}" title="${escapeHtml(orderProgress.label)}">→ OBJ</span>`
    : "";
  const priceText = formatMoney(item.price, item.currency);
  const priceBadge = priceText === "Bez ceny" ? priceText : `${priceText} / ks`;
  return `<article class="offer-item offer-item-catalog ${soldOut ? "sold-out" : ""} ${orderProgress.state ? `is-order-${orderProgress.state}` : ""}">
    <div class="offer-item-main offer-item-main-catalog">
      ...
      <div class="offer-item-copy">
        <div class="offer-item-copy-top">
          <div class="offer-item-heading">
            <strong>${escapeHtml(displayName)}</strong>
          </div>
          <span class="row-actions offer-item-tools">
            ${orderFlag}
        ...
        <div class="pills offer-stock offer-stock-catalog">
        ...
        <div class="offer-item-footer">
          <span class="offer-item-price-row">
            <span class="offer-qty-tag">${escapeHtml(`${total} ks`)}</span>
            <span class="offer-price-tag">${escapeHtml(priceBadge)}</span>
          </span>
        </div>
      </div>
    </div>
    <div class="offer-item-cta-row">
      <button class="button primary" type="button" data-add-reservation="${item.id}" ${available <= 0 ? "disabled" : ""}>Rezervovat</button>
      <button class="button" type="button" title="Přidat náhradníka" data-add-reservation="${item.id}" data-add-reservation-status="alternate">Náhradník</button>
    </div>
...
function deleteCustomer(id) {
  ...
  const customerOrders = state.data.orders.filter((item) => item.customerId === id);
  pushTrashEntry("customer-bundle", customerName(customer), {
    customer,
  ...
  customerOrders.forEach((order) => state.selectedOrderIds.delete(order.id));
  state.selectedCustomerId = state.data.customers[0]?.id || null;
  saveData();
  renderAll();
  toast("Zákazník přesunut do koše.");
}
...
function deleteOrder(id) {
  ...
  const customer = findCustomer(order.customerId);
  pushTrashEntry("order", [customerName(customer), clean(order.orderDate)].filter(Boolean).join(" · ") || "Objednávka", {
    order,
  });
  ...
  saveData();
  renderAll();
  toast("Objednávka přesunuta do koše.");
}
...
async function saveVarietyFromForm() {
  ...
  const photoUrl = uploadedImages[0] || (allImages.includes(chosenMainPhoto) ? chosenMainPhoto : "") || explicitPhotoUrl || allImages[0] || "";
...
function openCrossDialog(id = null) {
  ...
  const seedlingImages = crossSeedlingImages(cross);
  const seedlingMainPhoto = crossSeedlingMainPhoto(cross) || seedlingImages[0] || "";
  els.crossForm.elements.seedlingPhotoUrl.value = seedlingMainPhoto;
  els.crossForm.elements.seedlingGallery.value = seedlingImages.filter((image) => image !== seedlingMainPhoto).join("\n");
  els.crossForm.elements.seedlingMainPhoto.value = seedlingMainPhoto;
...
async function saveCrossFromForm() {
  ...
  const existingImages = unique([
    ...
    ...crossOwnSeedlingImages(existing),
  ...
  const seedlingPhotoUrl = uploadedMain || uploaded[0] || (allSeedlingImages.includes(mainToken) ? mainToken : "") || allSeedlingImages[0] || "";
...
function ensureVarietyFromCross(cross) {
  ...
  const seedlingImages = crossOwnSeedlingImages(cross);
  const mainPhoto = seedlingImages[0] || "";
...
function deleteCross(id) {
  ...
  pushTrashEntry("cross", clean(cross.seedlingName) || "Křížení", { cross });
  ...
  saveData();
  renderAll();
  toast("Křížení přesunuto do koše.");
}
...
function deleteVariety(id) {
  ...
  pushTrashEntry("variety", variety.name, {
    variety: normalizeVarietySnapshot(variety),
  });
  state.data.varieties = state.data.varieties.filter((item) => item.id !== id);
  saveData();
  renderAll();
  toast("Odrůda přesunuta do koše.");
}
...
async function saveOfferItemFromForm() {
  ...
  const now = new Date().toISOString();
  if (variety && uploaded[0]) {
    const previousImages = varietyImages(variety);
    variety.photoUrl = uploaded[0];
    variety.gallery = unique(previousImages.filter((image) => image !== uploaded[0]));
  ...
  const item = normalizeOfferItem({
    ...
    photoUrl: variety ? "" : (uploaded[0] || existing?.photoUrl || ""),
    note: form.get("note"),
...
function deleteOfferItem(offerId, itemId) {
  ...
  pushTrashEntry("offer-item", offerItemNameSafe(item), {
    offerId: offer.id,
  ...
  saveData();
  renderAll();
  toast("Položka přesunuta do koše.");
}
...
function openReservationDialog(offerId, itemId, reservationId = null, defaults = {}) {
  ...
  els.reservationForm.elements.customerId.value = reservation?.customerId || "";
...
async function materializeDataPhotos(data) {
  ...
  for (const cross of data.crosses || []) {
    const images = await materializePhotoList(crossOwnSeedlingImages(cross));
...
async function buildMobilePhotoPayload(type) {
  ...
  for (const cross of state.data.crosses || []) {
    crosses.push({
      ...
      seedlingPhotos: await materializePhotoList(crossOwnSeedlingImages(cross)),
...
function mergeImagesIntoCross(cross, images) {
  ...
  const before = crossOwnSeedlingImages(cross);
  const merged = unique([...before, ...images]);
...
async function writeSupabaseSyncSafely(session, encryptedData, expectedUpdatedAt, updatedAt) {
...
async function pushSupabaseSync(options = {}) {
  ...
  try {
    ...
    if (cloudState.data) {
      rememberSupabaseSyncSnapshot(SUPABASE_SYNC_LAST_CLOUD_SNAPSHOT_KEY, "cloud-before-push", cloudState.data, cloudState.updatedAt);
      rememberSupabaseSyncSnapshot(SUPABASE_SYNC_LAST_LOCAL_SNAPSHOT_KEY, "local-before-merge", state.data);
    ...
    await writeSupabaseSyncSafely(session, encrypted, cloudState.updatedAt, updatedAt);
    state.syncEncryptionVerifiedPassword = encryptionPassword;
...
async function pullSupabaseSync(options = {}) {
  try {
    ...
    state.syncEncryptionVerifiedPassword = encryptionPassword;
    const cloudData = normalizeLoadedData(decrypted);
...
async function buildSupabaseSyncData(userId) {
  ...
  for (const variety of data.varieties || []) {
    const images = varietyImages(variety);
    const refs = await uploadPhotoListForCloudRecordSync(userId, variety.name || "odruda", images);
  ...
  for (const cross of data.crosses || []) {
    ...
    const refs = await uploadPhotoListForCloudRecordSync(userId, cross.seedlingName || "semenac", images);
  ...
  for (const offer of data.offers || []) {
    for (const item of offer.items || []) {
      ...
      item.photoUrl = (await uploadPhotoListForCloudRecordSync(userId, offerItemNameSafe(item) || "nabidka", originalPhoto ? [originalPhoto] : []))[0] || "";
...
async function uploadPhotoListForCloudRecordSync(userId, ownerName, images) {
  const originalRefs = unique((images || []).map(clean).filter(Boolean));
  try {
    const refs = await uploadPhotoListForSync(userId, ownerName, originalRefs);
    ensureSupabasePhotoRefsForSync(originalRefs, refs, ownerName || "fotka");
    return refs;
  } catch (error) {
    console.warn("Photo sync skipped, record will continue without broken local photo", ownerName, error);
    return originalRefs.filter((ref) => isSupabasePhotoRef(ref));
  }
}
...
async function photoToOriginalFile(image, ownerName = "fotka") {
  ...
  try {
    ...
    if (isSupabasePhotoRef(value)) {
      const original = await resolveSupabasePhotoFile(value);
      if (original) return original;
      const thumbnailRef = supabaseThumbnailRef(value);
      return thumbnailRef !== value ? await resolveSupabasePhotoFile(thumbnailRef) : null;
    }
...
async function resolvePhotoPreviewUrl(ref) {
  const value = clean(ref);
  if (!value) return "";
  if (isLocalPhotoRef(value)) return resolveLocalPhotoUrl(value);
  if (isIndexedPhotoRef(value)) return resolveIndexedPhotoUrl(value);
  if (isSupabasePhotoRef(value)) return resolveSupabasePhotoUrl(supabaseThumbnailRef(value));
  return value;
}
...
async function tryNextPhotoFallback(image) {
  ...
  const url = await resolvePhotoPreviewUrl(nextRef);
...
function galleryMainContent(images, varietyName, index) {
  ...
  return `
    ...
    ${photoImageMarkup(images[index], varietyName, "", `loading="eager" ${index === 0 ? photoFallbackAttributes(variety, images) : ""}`)}
...
async function downloadPhoto(image, varietyName, index = 0) {
  ...
  try {
    ...
    if (isLocalPhotoRef(image)) {
      ...
      if (!file) {
        const thumbnailRef = supabaseThumbnailRef(image);
        if (thumbnailRef !== image) file = await resolveSupabasePhotoFile(thumbnailRef);
      }
      if (file) href = URL.createObjectURL(file);
    }
    if (isSupabasePhotoRef(image) && !file) throw new Error("Photo is not available.");
...
function offerItemImage(item = {}) {
  ...
  if (clean(canonical)) return clean(canonical);
  const variety = findVariety(clean(item.varietyId)) || findVarietyByName(item.varietyName);
  return varietyImages(variety)[0] || clean(item.photoUrl) || "";
}
...
function renderOfferDetail() {
  ...
  els.offerDetail.innerHTML = `
    ...
    <section class="detail-section">
      <div class="stack-list offer-items-catalog-list">
        ${items.length ? items.map((item) => renderOfferItem(offer, item)).join("") : emptyState("Zatím bez odřezků.")}
      </div>
    </section>
  `;
